...
static void start_zircon_64(uint64_t entry, void* bootdata) {
#if __x86_64__
...
extern void jump_to_legacy_kernel(void *entry, void *bootdata);
static void start_zircon_32(uint64_t entry, void* bootdata) {
#if __x86_64__
    jump_to_legacy_kernel(entry, bootdata);
#else
#warning "add code for other arches here"
#endif
    for (;;)
...
static int header_check(void* image, size_t sz, uint64_t* _entry,
                        size_t* _flen, size_t* _klen, uint32_t *flag) {
    bootdata_t* bd = image;
    size_t flen, klen;
    uint64_t entry;
    int align;
    ...
    if ((sz < sizeof(zircon_kernel_t)) ||
        ...
        printf("boot: invalid kernel header\n");
        return -1;
    }
    align = (kernel->hdr_kernel.flags & BOOTDATA_FLAG_X32)? 4: 8;
    flen = BOOTDATA_ALIGN_UP(kernel->hdr_file.length, align);
    klen = BOOTDATA_ALIGN_UP(kernel->hdr_kernel.length, align);
    entry = kernel->data_kernel.entry64;
    if (flen > (sz - sizeof(bootdata_t))) {
        printf("boot: invalid kernel header (bad flen)\n");
        return -1;
    }

    if (klen > (sz - (sizeof(bootdata_t) * 2))) {
        printf("boot: invalid kernel header (bad klen)\n");
        return -1;
    }
    if (_entry) {
        *_entry = entry;
...
static int item_check(bootdata_t* bd, size_t sz) {
    ...
    if ((bd->magic != BOOTITEM_MAGIC) ||
        ...
        (BOOTDATA_ALIGN_UP(bd->length, align) > sz)) {
        return -1;
    } else {
        return 0;
    }
...
unsigned identify_image(void* image, size_t sz) {
    ...
    while (sz > sizeof(bootdata_t)) {
        ...
        align = (bd->flags & BOOTDATA_FLAG_X32)? 4: 8;
        image += BOOTDATA_ALIGN_UP(bd->length, align) + sizeof(bootdata_t);
        sz -= BOOTDATA_ALIGN_UP(bd->length, align);
        n++;
    }
...
int boot_zircon(efi_handle img, efi_system_table* sys,
    ...
    // jump to the kernel
    //printf("jump to entry 0x%lx\r\n", entry);
    start_zircon_64(entry, ramdisk - FRONT_BYTES);
...
int zedboot(efi_handle img, efi_system_table* sys,
    ...
    size_t flen, klen;
    if (header_check(image, sz, NULL, &flen, &klen, NULL)) {
        return -1;
    }
...
int boot_core(efi_handle img, efi_system_table* sys, void* image, size_t isz)
{
    uint32_t x32 = 0;
    uint64_t entry;
    efi_status r;
    ...
    if ((bd->type == BOOTDATA_CONTAINER) &&
        ...
        void *bootdata = NULL;
        r = gBS->AllocatePages(AllocateAnyPages, EfiLoaderData, pages, (efi_physical_addr *)&bootdata);
        if (r) {
        ...
        //
        if(x32)
            start_zircon_32(entry, bootdata);
        else
            start_zircon_64(entry, bootdata);
